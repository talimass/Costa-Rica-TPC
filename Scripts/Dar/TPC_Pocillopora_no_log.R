# =========================================================
# POCILLOPORA rTPC FITTING SCRIPT
...
# Input: POCILLOPORA_Photo.T.xlsx or POCILLOPORA_Photo.T.csv
# =========================================================
# This script reads the POCILLOPORA Photo.T file, reformats the data for rTPC,
# fits the same TPC model to each curve, extracts fitted parameters with
# calc_params(), creates predictions, and exports tables and styled plots.
...
if (file.exists(xlsx_file)) {
  ...
  stop("Could not find POCILLOPORA_Photo.T.xlsx or POCILLOPORA_Photo.T.csv in: ", in_dir)
}
...
write.csv(
  ...
  file.path(out_dir, "POCILLOPORA_quadratic_all_fits.csv"),
  row.names = FALSE
)
...
write.csv(
  best_tbl %>%
    select(curve_id, Site, fragment_ID, rate.type, fitted_model, n_points, n_temps, AIC, BIC, sigma, nobs),
  file.path(out_dir, "POCILLOPORA_quadratic_successful_fits.csv"),
  row.names = FALSE
)
...
write.csv(
  failed_tbl,
  file.path(out_dir, "POCILLOPORA_quadratic_failed_fits.csv"),
  row.names = FALSE
)
...
write.csv(
  coef_tbl,
  file.path(out_dir, "POCILLOPORA_quadratic_coefficients.csv"),
  row.names = FALSE
)
...
write.csv(
  params_tbl,
  file.path(out_dir, "POCILLOPORA_quadratic_calc_params.csv"),
  row.names = FALSE
)
...
write.csv(
  pred_tbl,
  file.path(out_dir, "POCILLOPORA_quadratic_predictions.csv"),
  row.names = FALSE
)
...
write_xlsx(
  list(
    raw_data = dat,
    successful_fits = best_tbl %>% select(curve_id, Site, fragment_ID, rate.type, fitted_model, n_points, n_temps, AIC, BIC, sigma, nobs),
    failed_fits = failed_tbl,
    coefficients = coef_tbl,
    calc_params = params_tbl,
    predictions = pred_tbl
  ),
  file.path(out_dir, "POCILLOPORA_quadratic_rTPC_outputs.xlsx")
)

# ----------------------------
# PLOT HELPER
# ----------------------------
# Plot raw points and the fitted quadratic rTPC curve for one rate type.
# White background, no model-name text inside the plot, no log scale.
...
p_np <- make_rtpc_plot(
  rate_name = "NP",
  plot_title = "Pocillopora NP fitted with quadratic_2008",
  out_name = "POCILLOPORA_quadratic_NP_by_fragment.png"
)

p_r <- make_rtpc_plot(
  rate_name = "R",
  plot_title = "Pocillopora R fitted with quadratic_2008",
  out_name = "POCILLOPORA_quadratic_R_by_fragment.png"
)

p_gp <- make_rtpc_plot(
  rate_name = "GP",
  plot_title = "Pocillopora GP fitted with quadratic_2008",
  out_name = "POCILLOPORA_quadratic_GP_by_fragment.png"
)
